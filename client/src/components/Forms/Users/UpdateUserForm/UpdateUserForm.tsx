...
import { DatePicker, Select, Form, Input, Spin } from 'antd';
...
import { UpdateUserFormSchema } from '../../../../utils/validationSchemes';


export const UpdateUserForm: React.FC<{ data: UserInfoDto }> = ({ data }) => {
    const { id } = useParams();
    const [UpdateUserByID, { isLoading: isUserUpdating }] = useUpdateUserByIDMutation();
    ...
    const formik = useFormik({
        initialValues: {
            ...
            underground: data.underground?.id ?? '',
        },
        initialStatus: false,
        ...
        <Spin spinning={isUserUpdating}>
            <Form layout="vertical" onSubmitCapture={formik.handleSubmit}>
                ...
                <Form.Item label="Почта">
                    <Input name='email' placeholder="Почта" value={formik.values.email} onChange={formik.handleChange} />
                    {formik.errors.email ? <span className="form__err-msg">{formik.errors.email}</span> : null}
                </Form.Item>
                <Form.Item label="Адрес">
                    <Input name='address' placeholder="Адрес" value={formik.values.address} onChange={formik.handleChange} />
                    {formik.errors.address ? <span className="form__err-msg">{formik.errors.address}</span> : null}
                </Form.Item>
                <Form.Item label="Телефон">
                    <Input name='phone' placeholder="+79008003510" value={formik.values.phone} onChange={formik.handleChange} />
